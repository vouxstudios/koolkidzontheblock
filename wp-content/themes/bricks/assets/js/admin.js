				...
				if (action === 'bricks_deactivate_license') {
					...
					if (response.success) {
						if (response.data.hasOwnProperty('message')) {
							licenseKeyForm.querySelector('.success-message').textContent = response.data.message
						}
						...
						if (response.data.hasOwnProperty('message')) {
							licenseKeyForm.querySelector('.error-message').textContent = response.data.message
						}
					}
				}
			}
		...
		revalidateButton.addEventListener('click', function (e) {
			...
			jQuery.ajax({
				...
				error: function (xhr, status, error) {
					revalidateButton.disabled = false
					revalidateButton.value = originalText
					errorMessage.textContent = 'An error occurred while re-validating the license.'
				}
			})
		})
	}
